...
export default function DriftCheck() {
  ...
  async function onRun() {
    ...
    setError("");
    try {
      setResult(await checkDrift(refData, curData));
    } catch (e) {
  ...
  return (
    <div className="card">
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div className="row">
          <div>
            ...
            />
            {curFile && <p className="muted">Файл: {curFile.name}</p>}
          </div>
          <div>
            <label>
              <b>Новые данные:</b>
      ...
      <button onClick={onRun} disabled={busy || !refData || !curData}>
        Проверить дрейф
      </button>
        ...
        <div style={{ marginTop: 12 }}>
          <p>
            ...
            {result.overall_drift ? (
              <span className="error">🔴 Есть дрейф</span>
            ) : (
              <span className="ok">🟢 Нет дрейфа</span>
            )}
          </p>
          <p>
            Процент дрейфа: <b>{result.drift_percentage}%</b>
          </p>
      ...
      <div style={{ marginTop: 24 }}>
        ...
        {metrics.length > 0 ? (
          <Plot
            data={[
              {
                x: metrics.map((_, i) => i + 1),
                y: metrics.map((m) => m.f1 ?? 0),
                name: "F1",
                mode: "lines+markers",
                type: "scatter",
              },
            ...
            layout={{
              title: "Метрики по запускам",
              xaxis: { title: "Запуск" },
              yaxis: { title: "Метрика", range: [0, 1] },
              height: 400,
              showlegend: true,
          ...
          <p className="muted">
            Нет данных о метриках. Запустите анализ на главной.
          </p>
        )}
      </div>
    </div>
