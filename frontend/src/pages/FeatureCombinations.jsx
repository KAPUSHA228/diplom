...
export default function FeatureCombinations() {
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Failed to parse last_analysis_result:", err.message);
    }
  }, []);
  ...
  useEffect(() => {
    if (activeData?.length) {
      ...
      setAllCols(cols);
      setNumericalCols(
        cols.filter((c) => typeof activeData[0][c] === "number"),
      );
  ...
  async function onRun() {
    ...
    setSelectedRecs([]);
    try {
      const res = await createFeatureCombinations(
        activeData,
        numericalCols,
        textCols,
        maxPairs,
      ...
      if (res.recommendations)
        setSelectedRecs(res.recommendations.map((r) => r.name));
    } catch (err) {
          ...
          >
            <label>
              ...
              >
                ...
                {allCols
                  .filter((c) => typeof activeData[0][c] === "number")
                  .map((c) => (
        ...
        >
            ...
            >
              {result.recommendations.map((rec) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={selectedRecs.includes(rec.name)}
                    onChange={(e) =>
                      setSelectedRecs((prev) =>
                        e.target.checked
                          ? [...prev, rec.name]
                          : prev.filter((x) => x !== rec.name),
                      )
                    }
                  />
                  ...
                  <span
                    style={{
                      marginLeft: "auto",
                      color: rec.correlation > 0 ? "#2ecc71" : "#e74c3c",
                      fontWeight: "bold",
                    }}
                  >
                    r = {rec.correlation > 0 ? "+" : ""}
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 6,
              border: "none",
              background: selectedRecs.length ? "var(--primary)" : "#666",
              color: "#fff",
              cursor: "pointer",
            }}
          >
