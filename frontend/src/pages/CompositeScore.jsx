...
export default function CompositeScore() {
  ...
  const numericCols = useMemo(() => {
    return activeData?.length
      ? Object.keys(activeData[0]).filter(
          (k) => typeof activeData[0][k] === "number",
        )
      : [];
  }, [activeData]);
  ...
  async function onFileChange(e) {
    ...
    try {
      ...
      if (parsed?.allData?.length) {
        const w = {};
        Object.keys(parsed.allData[0])
          .filter((k) => typeof parsed.allData[0][k] === "number")
          .forEach((c) => {
            w[c] = 1;
          });
        setWeights(w);
  ...
  async function onRun() {
    ...
    try {
      ...
      Object.entries(weights).forEach(([k, v]) => {
        cleanWeights[k] = Number(v) || 0;
      });
      const res = await createCompositeScore(
        activeData,
        cleanWeights,
        scoreName,
      );
          ...
          <div style={{ marginTop: 12 }}>
            <label>
              ...
              <span className="muted" style={{ fontSize: 12 }}>
                (можно вводить дробные, например 0.5 или -1.2)
              </span>
            </label>
                  ...
                  data={[
                    {
                      ...
                      type: "bar",
                      orientation: "h",
                      text: Object.values(result.correlations)
                        .slice(0, 5)
                        .map((v) => v.toFixed(2)),
                      textposition: "outside",
                      textfont: { size: 14, color: "#fff" },
                      marker: {
                        color: Object.values(result.correlations)
                          .slice(0, 5)
                          .map((v) => (v > 0 ? "#2ecc71" : "#e74c3c")),
                        opacity: 0.9,
                      },
                    },
                  ]}
