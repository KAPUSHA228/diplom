...
class TestPreprocessData:
    ...
    def test_no_nan_in_X(self, sample_students_with_nans):
        ...
        X, y = preprocess_data(sample_students_with_nans, feature_cols, "risk_flag", use_smote=False)
...
class TestSelectFeaturesForModel:
    ...
    def test_selected_names_are_subset(self, sample_students):
        X = sample_students[["avg_grade", "stress_level", "satisfaction_score", "motivation_score", "n_essays"]]
        y = sample_students["risk_flag"]
...
class TestCreateFeatureCombinations:
    ...
    def test_num_plus_num_creates_new_columns(self):
        ...
        result = create_feature_combinations(df, numerical_cols=["a", "b"], text_cols=[], methods=["sum", "diff"])
        # Новые колонки должны появиться
        assert len(result.columns) > len(df.columns)

    def test_no_infinite_values(self):
        df = pd.DataFrame({"a": [1, 2, 0], "b": [4, 0, 6]})
        result = create_feature_combinations(df, numerical_cols=["a", "b"], text_cols=[], methods=["ratio"])
    ...
    def test_empty_numerical_cols(self):
        ...
        result = create_feature_combinations(df, numerical_cols=[], text_cols=["text"], methods=["sum"])
