...
class ExperimentTracker:
        ...
        if "explanations" in data and data["explanations"]:
            if isinstance(data["explanations"], list):
                pd.DataFrame(data["explanations"]).to_csv(
                    os.path.join(exp_dir, "explanations.csv"), index=False, encoding="utf-8"
                )

        # Сохраняем предсказания (если есть)
        if "predictions" in data and data["predictions"] is not None:
            data["predictions"].to_csv(os.path.join(exp_dir, "predictions.csv"), index=False, encoding="utf-8")
    ...
    def load_experiment(self, experiment_id: str) -> Dict[str, Any]:
        """
        Загружает сохранённый эксперимент (метаданные, модель, объяснения, предсказания).

        Args:
            experiment_id: идентификатор эксперимента (возвращается из save_experiment)

        Returns:
            dict: метаданные + опционально model, explanations, predictions, params
        """
    ...
    def list_experiments(self, limit: int = 20) -> pd.DataFrame:
        """
        Возвращает список сохранённых экспериментов (отсортирован по дате).

        Args:
            limit: максимальное количество экспериментов (по умолчанию 20)

        Returns:
            pd.DataFrame: таблица с id, name, timestamp, model_name, metrics
        """
        experiments = []
