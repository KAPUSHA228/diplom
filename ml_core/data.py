...
def save_synthetic_data(df, filename=None, directory=None):
    """
    ...
    if directory is None:
        directory = config.ANALYSIS_DATA_DIR / "synthetic"
...
def save_data_for_monitoring(df, feature_cols, semester="spring_2026", directory=None):
    """
    Сохраняет только признаки для мониторинга дрейфа распределений.

    Args:
        df: исходный DataFrame
        feature_cols: список имён признаков для сохранения
        semester: название семестра (для имени файла)
        directory: директория для сохранения (по умолчанию: analysis_data/monitoring)
    ...
    df = df.copy()
    if directory is None:
        directory = config.ANALYSIS_DATA_DIR / "monitoring"
    os.makedirs(directory, exist_ok=True)
...
def load_data(category: str = "grades", n_students: int = 500, generate_two_sets: bool = False):
    """
    Загружает синтетические данные для выбранной категории (кэшируется).

    Args:
        category: категория данных (grades, psychology, creativity, values,
                  personality, activities, career)
        n_students: количество студентов (по умолчанию 500)
        generate_two_sets: если True — возвращает reference и new данные для дрейфа

    Returns:
        pd.DataFrame | tuple: DataFrame (или tuple из двух DataFrame если generate_two_sets=True)
    """
...
def _generate_synthetic_data_tz():
    ...
    demographics = pd.DataFrame(
        {
            ...
            "field_of_study": np.random.choice(
                ["Социология", "Психология", "Экономика", "Политология", "Социальная работа"], n_students
            ),
        }
    )
    ...
    activities["activity_score"] = activities[
        ["research_projects", "sports", "volunteering", "creative_events", "student_council"]
    ].sum(axis=1)
...
def _aggregate_student_features(grades_df, questionnaires_df, psych_tests_df, essays_df):
    ...
    grades_agg = (
        ...
        .agg({"grade": ["mean", "std", "min", "max", "count"], "brs_score": "mean"})
        .reset_index()
    )
...
def load_from_csv(file):
    ...
    Args:
        file: путь к файлу или UploadedFile из Streamlit
...
def generate_temporal_data(n_students=500, n_semesters=3):
    """
    Генерация данных с временной динамикой (несколько семестров на студента).

    Args:
        n_students: количество студентов
        n_semesters: число семестров

    Returns:
        pd.DataFrame: данные с колонками student_id, semester, avg_grade, ...
    """
    np.random.seed(42)
...
def prepare_data_for_training(df, feature_cols, target_col="risk_flag", test_size=0.2):
    """
    Разделение на train/test + заполнение пропусков медианой.
...
def generate_synthetic_data_by_category(category: str, n_students: int = 500, generate_two_sets: bool = False) -> dict:
    """
    Генерирует синтетические данные для выбранной категории.

    Args:
        category: категория (grades, psychology, creativity, values,
                  personality, activities, career)
        n_students: количество студентов (по умолчанию 500)
        generate_two_sets: если True — создаёт reference + new данные для дрейфа

    Returns:
        dict: {'data': DataFrame, 'target': Series, 'target_col': str, ...}
    """
    np.random.seed(42)
...
def _generate_grades_category(n_students: int, generate_two_sets: bool = False) -> dict:
    ...
    result = {"data": data, "target": "risk_flag", "target_values": data["risk_flag"]}
    ...
    if generate_two_sets:
        ...
        new_data["attendance_rate"] = np.clip(
            new_data["attendance_rate"] - np.random.normal(0.1, 0.05, n_students), 0.4, 1.0
        )
        new_data["risk_flag"] = (
...
def _generate_psychology_category(n_students: int, generate_two_sets: bool = False) -> dict:
    ...
    result = {"data": data, "target": "burnout_risk", "target_values": data["burnout_risk"]}
    ...
    if generate_two_sets:
        ...
        new_data["motivation_score"] = np.clip(
            new_data["motivation_score"] - np.random.normal(0.3, 0.2, n_students), 1, 10
        )
...
def _generate_creativity_category(n_students: int, generate_two_sets: bool = False) -> dict:
    ...
    result = {"data": data, "target": "high_creativity", "target_values": data["high_creativity"]}

    if generate_two_sets:
        np.random.seed(43)
        new_data = data.copy()
        # Имитируем дрейф: общая креативность снижается
        new_data["creativity_total"] = np.clip(
            new_data["creativity_total"] - np.random.normal(15, 10, n_students), 40, 160
        )
...
def _generate_values_category(n_students: int, generate_two_sets: bool = False) -> dict:
    ...
    result = {"data": data, "target": "value_profile", "target_values": data["value_profile"]}
...
def _generate_personality_category(n_students: int, generate_two_sets: bool = False) -> dict:
    ...
    result = {"data": data, "target": "leadership_potential", "target_values": data["leadership_potential"]}
...
def _generate_activities_category(n_students: int, generate_two_sets: bool = False) -> dict:
    ...
    data["activity_score"] = data[
        ["research_projects", "sports", "volunteering", "creative_events", "student_council", "conferences"]
    ].sum(axis=1)
    data["active_participation"] = (data["activity_score"] >= 3).astype(int)

    result = {"data": data, "target": "active_participation", "target_values": data["active_participation"]}
    ...
    if generate_two_sets:
        ...
        new_data["activity_score"] = new_data[
            ["research_projects", "sports", "volunteering", "creative_events", "student_council", "conferences"]
        ].sum(axis=1)
...
def _generate_career_category(n_students: int, generate_two_sets: bool = False) -> dict:
    ...
    data = pd.DataFrame(
        {
            ...
            "works_now": np.random.choice(
                ["Да, по специальности", "Да, не по специальности", "Нет"], n_students, p=[0.2, 0.3, 0.5]
            ),
        }
    )
    ...
    result = {"data": data, "target": "career_clarity", "target_values": data["career_clarity"]}
